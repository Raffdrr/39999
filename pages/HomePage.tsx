...
import { Heart, Star, Users, MapPin as MapPinIconLucide, AlertTriangle, Gift, ListFilter, Search as SearchIcon, X } from 'lucide-react';
                        ...
                        {filteredLocali.map((l, index) => (
                            <ListCard key={l.id} onClick={() => openModal('selectedLocale', l.id)} itemType="locale" index={index}>
                                ...
                                <div className="flex flex-col justify-between py-1 flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center text-xs sm:text-sm text-amber-600 font-bold mt-2">
                           ...
                           <ListCard key={e.id} onClick={() => openModal('selectedEvent', e.id)} itemType="event" index={index} isCharity={e.isCharityEvent}>
                               ...
                               <div className="flex flex-col justify-between py-1 flex-1 min-w-0">
                                   ...
                                   <div className="text-xs text-slate-600 mt-2">
                                       {e.partecipationFee ? <span className="font-semibold text-green-600">{e.partecipationFee}</span> : <span className="font-semibold text-green-600">Gratuito</span>}
                                   </div>
                               </div>
