...
interface PayWithCreditAmountModalProps {
  data: PayWithCreditAmountModalData;
  onClose: () => void;
}
...
const PayWithCreditAmountModal: React.FC<PayWithCreditAmountModalProps> = ({ 
  ...
  useEffect(() => {
    if (data) {
      ...
      } else { // Locale bill contribution
        setAmountToPay(""); 
      }
    }
  }, [data]);

  const onConfirmPayment = (itemType: 'locale' | 'event', itemId: string, amount: number, isEventFee?: boolean) => {
    if (isEventFee) {
      payEventFeeWithCredit(itemId, amount);
    } else {
      applyCreditToBill(itemId, amount);
    }
  };
  ...
  return (
    <ModalWrapper open={!!data} onClose={onClose} customClasses="max-w-sm w-full">
      <form onSubmit={handleSubmit} className="bg-gradient-to-br from-orange-100 via-yellow-50 to-emerald-100 dark:bg-black rounded-2xl shadow-2xl w-full max-h-[90vh] flex flex-col">
        <div className="p-4 sm:p-5 border-b border-emerald-200 dark:border-emerald-500/20 flex justify-between items-center sticky top-0 bg-white/80 dark:bg-black/80 backdrop-blur-sm rounded-t-2xl z-10">
        ...
        <div className="overflow-y-auto p-4 sm:p-5 space-y-3 sm:space-y-4">
          ...
          <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-300">
            Credito disponibile: <span className="font-bold text-emerald-600 dark:text-emerald-400">€{credit.toFixed(2)}</span>
          </p>
          <div>
            <label htmlFor="amountToPay" className="block text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
            ...
            <input
              ...
              step="0.01"
              min="0.01"
              max={maxAllowedInput > 0 ? maxAllowedInput : undefined}
              required
              placeholder={placeholderText}
              className={`form-input focus:ring-emerald-500 focus:border-emerald-500 ${data.isEventFee && !!data.maxAmount ? 'bg-slate-100 dark:bg-slate-700' : ''}`}
              autoFocus
            />
          </div>
           {maxAllowedInput <= 0 && !data.isEventFee && (
                <p className="text-[10px] text-orange-600 dark:text-orange-400">Il conto è già coperto o non hai credito sufficiente per il rimanente.</p>
           )}
           {amountToPay && parseFloat(amountToPay) > maxAllowedInput && maxAllowedInput > 0 && (
             <p className="text-[10px] text-red-600 dark:text-red-400">L'importo supera il massimo applicabile di €{maxAllowedInput.toFixed(2)}.</p>
           )}
        </div>
        <div className="p-4 sm:p-5 border-t border-emerald-200 dark:border-emerald-500/20 mt-auto sticky bottom-0 bg-white/80 dark:bg-black/80 backdrop-blur-sm rounded-b-2xl">
          <button
            type="submit"
