    ...
    <ModalWrapper open={true} onClose={onClose} customClasses="max-w-md w-full">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-h-[80vh] flex flex-col">
        ...
        <div className="overflow-y-auto p-4 sm:p-5 space-y-2.5 sm:space-y-3">
          <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 mb-2.5 sm:mb-3">Seleziona gli amici da invitare al tuo evento. Vengono mostrati amici (simulati) nelle vicinanze.</p>
          {NEARBY_FRIENDS_DATA.map((friend: FriendData) => (
            ...
            >
              <div className="flex items-center gap-2.5 sm:gap-3">
                ...
                <div>
                  <p className="font-medium text-sm sm:text-base text-slate-700 dark:text-slate-200">{friend.name}</p>
                  <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400">~ {friend.distance} da te</p>
                </div>
              </div>
